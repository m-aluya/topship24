...
class Topship_API_Service_Africa {
    ...
    public static function handle_registration($request) {
        ...
        if (isset($resultData['message']) && $resultData['message'] == 'User already exists! Please sign in') {
            if (Class_topship_helper::loginWithDetails($email,$password)!=null){
                global $wpdb;
                $wpdb->insert($wpdb->prefix . 'top_ship_registrations', [
                    'email' => $email,
   ...
   /* public static function handle_registration($request) {
        ...
        $jsonData = wp_json_encode($formData);

        // Log the JSON encoded data
        error_log('JSON encoded Topship registration data: ' . $jsonData);

        // Send POST request using wp_remote_post
        $signupUrl = 'https://topship-staging.africa/api/signup';
        $response = wp_remote_post($signupUrl, [
            'method'      => 'POST',
            'body'        => $jsonData,
            'headers'     => [
                'Content-Type' => 'application/json',
        ...
        $resultData = json_decode(wp_remote_retrieve_body($response), true);
        ...
        if (isset($resultData['message']) && $resultData['message'] == 'User already exists! Please sign in') {
            ...
            // Registration success; store user data in WordPress database
            global $wpdb;
            $wpdb->insert($wpdb->prefix . 'top_ship_registrations', [
                'email' => $email,
                'password' => self::encrypt($password),
